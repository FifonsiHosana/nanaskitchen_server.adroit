...
export const attributionAndPreferences = mysqlTable(
...
export const countries = mysqlTable(
  "Countries",
  {
    ...
    currencyId: int("currency_id").references(() => currency.id, {
      onDelete: "set null",
      onUpdate: "cascade",
    }),
  },
...
export const countryProductSettings = mysqlTable(
...
export const deliveryLocation = mysqlTable(
  "DeliveryLocation",
  {
    id: int().autoincrement().notNull(),
...
export const feedBackAnswers = mysqlTable(
  "FeedBackAnswers",
  {
    id: int().autoincrement().notNull(),
    questionId: int().references(() => feedBackQuestions.id, {
      onDelete: "cascade",
      onUpdate: "cascade",
    }),
  ...
  (table) => [
    ...
    unique("FeedBackAnswers_referenceId_questionId_unique").on(
      table.referenceId,
      table.questionId,
    ),
  ],
...
export const feedBackQuestions = mysqlTable(
  "FeedBackQuestions",
  {
    id: int().autoincrement().notNull(),
...
export const flavors = mysqlTable(
  "Flavors",
  {
    id: int().autoincrement().notNull(),
  ...
  (table) => [primaryKey({ columns: [table.id], name: "Flavors_id" })],
);

export const openCloseOrders = mysqlTable(
  "OpenCloseOrders",
  {
    id: int().autoincrement().notNull(),
...
export const order = mysqlTable(
  "Order",
  {
    ...
    pricingGroupId: int().references(() => pricingGroups.id, {
      onDelete: "set null",
      onUpdate: "cascade",
    }),
  },
...
export const orderCartItem = mysqlTable(
  "OrderCartItem",
  {
    id: int().autoincrement().notNull(),
...
export const orderUserDetail = mysqlTable(
  "OrderUserDetail",
  {
    id: int().autoincrement().notNull(),
    orderId: int()
    ...
    deliveryLocationId: int().references(() => deliveryLocation.id, {
      onDelete: "set null",
      onUpdate: "cascade",
    }),
  },
...
export const packageSizes = mysqlTable(
  "PackageSizes",
  {
    id: int().autoincrement().notNull(),
    maxQty: int().notNull(),
  ...
  (table) => [primaryKey({ columns: [table.id], name: "PackageSizes_id" })],
);
...
export const platformSettings = mysqlTable(
  "PlatformSettings",
  {
    ...
    updatedAt: datetime({ mode: "string", fsp: 3 })
      .default(sql`(now())`)
      .notNull(),
    updatedBy: int().references(() => admin.id, {
      onDelete: "set null",
      onUpdate: "cascade",
    }),
  },
  (table) => [
    primaryKey({ columns: [table.id], name: "PlatformSettings_id" }),
    unique("PlatformSettings_key_unique").on(table.key),
  ],
);

export const pricingGroups = mysqlTable(
  "PricingGroups",
  {
    id: int().autoincrement().notNull(),
    groupName: varchar({ length: 191 }).notNull(),
  },
  (table) => [primaryKey({ columns: [table.id], name: "PricingGroups_id" })],
);

export const pricingTiers = mysqlTable(
  "PricingTiers",
  {
    id: int().autoincrement().notNull(),
    productId: int().references(() => product.id, {
...
export const product = mysqlTable(
  "Product",
  {
    ...
    updatedAt: datetime({ mode: "string", fsp: 3 }).notNull(),
    unitsPerCase: int(),
    flavorId: int().references(() => flavors.id, {
      onDelete: "set null",
      onUpdate: "cascade",
    }),
    isCase: boolean().default(false),
    variantId: int().references(() => variants.id, {
      onDelete: "set null",
      onUpdate: "cascade",
    }),
  },
...
export const variants = mysqlTable(
  ...
  (table) => [primaryKey({ columns: [table.id], name: "Variants_id" })],
);
...
export const auditLogs = mysqlTable(
  ...
  (table) => [primaryKey({ columns: [table.id], name: "audit_logs_id" })],
);

/**
 * Canonical RBAC resource list. Mirrors the `resource` enum on
 * `role_permissions` — keep the two in sync (additive changes need a migration).
 */
...
export const rolePermissions = mysqlTable(
  "role_permissions",
  {
    ...
    canDelete: boolean().default(false).notNull(),
  },
  (table) => [
    primaryKey({
      columns: [table.roleId, table.resource],
      name: "role_permissions_roleId_resource",
    }),
  ],
);
